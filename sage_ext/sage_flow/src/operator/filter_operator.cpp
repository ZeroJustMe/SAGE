...
FilterOperator::FilterOperator(std::string name)
    : Operator(OperatorType::kFilter, std::move(name)), filter_function_(nullptr) {}

FilterOperator::FilterOperator(std::string name, std::unique_ptr<FilterFunction> filter_function)
    : Operator(OperatorType::kFilter, std::move(name)), filter_function_(std::move(filter_function)) {}
...
auto FilterOperator::process(Response& input_record, int slot) -> bool {
  ...
  // Filter functions return the message if it passes, empty response if it doesn't
...
void FilterOperator::setFilterFunction(std::unique_ptr<FilterFunction> filter_function) {
