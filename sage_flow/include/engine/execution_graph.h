...
class ExecutionGraph {
 public:
  ...
  // Graph construction
  auto addOperator(std::shared_ptr<Operator> op) -> OperatorId;
  ...
  // Operator access
  auto getOperator(OperatorId id) -> std::shared_ptr<Operator>;
  auto getOperators() const -> const std::unordered_map<OperatorId, std::shared_ptr<Operator>>&;
 ...
 private:
  ...
  std::unordered_map<OperatorId, std::vector<OperatorId>> reverse_adjacency_list_;
