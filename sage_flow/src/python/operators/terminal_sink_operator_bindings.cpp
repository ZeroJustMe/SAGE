...
void BindTerminalSinkOperator(py::module& m) {
    ...
    m.def("SinkFunction", [](const std::function<void(const sage_flow::MultiModalMessage&)>& func) {
        return func;
    }, "Create a sink function for terminal output");
    
    // TerminalSinkOperator class binding
    py::class_<sage_flow::TerminalSinkOperator, sage_flow::Operator>(m, "TerminalSinkOperator")
